...
public sealed class CorrelatingMessageHandlerTests : IDisposable
{
    ...
    {
        _contextAccessor = new CorrelationContextAccessor { CorrelationContext = new CorrelationContext { CorrelationId = Guid.NewGuid().ToString() } };
        ...
        _httpClient = new HttpClient(_sut) { BaseAddress = BaseUri };
    }
    ...
    public async Task Given_correlationId_is_already_in_request_headers_should_not_overwrite()
    {
        ...
        // Act
        await _httpClient.SendAsync(new HttpRequestMessage { Headers = { { _correlateClientOptions.RequestHeader, existingCorrelationId } } });
