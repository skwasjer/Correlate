		...
		public async Task When_context_has_started_should_create_logScope_with_correlationId()
		{
			...
			{				
				...
				TestCorrelator.GetLogEventsFromCurrentContext()
					.Should().ContainSingle(le => le.MessageTemplate.Text.StartsWith("Setting response header"))
					.Which.Properties
					.Should().ContainSingle(p => p.Key == expectedLogProperty)
					.Which.Value
					.Should().BeOfType<ScalarValue>()
					.Which.Value
					.Should().Be(_correlationContext.CorrelationId);
			}
		}
